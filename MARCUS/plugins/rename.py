...
async def _(event):
    if event.fwd_from:
        return
    await event.edit("Renaming in process 馃檮馃檱鈥嶁檪锔忦煓団�嶁檪锔忦煓団�嶁檧锔� It might take some time if file size is big")
...
async def _(event):
    ...
    await event.edit("Rename & Upload in process 馃檮馃檱鈥嶁檪锔忦煓団�嶁檪锔忦煓団�嶁檧锔� It might take some time if file size is big")
    input_str = event.pattern_match.group(1)
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if event.reply_to_msg_id:
...
async def _(event):
    if event.fwd_from:
        return
    await event.edit("Rename & Upload as Streamable in process 馃檮馃檱鈥嶁檪锔忦煓団�嶁檪锔忦煓団�嶁檧锔� It might take some time if file size is big")
    input_str = event.pattern_match.group(1)
    if not os.path.isdir(Config.TMP_DOWNLOAD_DIRECTORY):
        os.makedirs(Config.TMP_DOWNLOAD_DIRECTORY)
    if event.reply_to_msg_id:
