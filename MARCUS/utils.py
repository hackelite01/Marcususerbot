...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        import importlib
        from pathlib import Path
        path = Path(f"MARCUS/plugins/{shortname}.py")
        name = "MARCUS.plugins.{}".format(shortname)
        ...
        # support for uniborg
        sys.modules["uniborg.util"] = MARCUS.utils
        mod.Config = Config
        mod.borg = bot
        # support for paperplaneextended
        sys.modules["MARCUS.events"] = MARCUS.utils
        spec.loader.exec_module(mod)
        # for imports
        sys.modules["MARCUS.plugins."+shortname] = mod
        print("Successfully imported "+shortname)
...
def remove_plugin(shortname):
    try:
        try:
            ...
            name = f"MARCUS.plugins.{shortname}"
...
def errors_handler(func):
    async def wrapper(errors):
        try:
            ...
            link = "[here](https://t.me/hackelitebotsupport)"
