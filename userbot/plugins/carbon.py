...
async def carbon_api(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        ...
        await e.edit("`Working On It Master?¥°")
        # Waiting for downloading
        sleep(2.5)
        color_name = driver.find_element_by_xpath(
            "/html/body/div[1]/main/div[3]/div[2]/div[1]/div[1]/div/span[2]/input"
        ).get_attribute("value")
        await e.edit("`Finished It Master?¥°\n100%`")
        file = "./carbon.png"
        await e.edit("`Uploading..It Master`")
        await e.client.send_file(
            e.chat_id,
            file,
            caption="<< `Here's your carbon Master!` \n **Carbonised Using** [GodHackerz-Userbot](https://github.com/hackelite01/Marcususerbot.git)>>\n**Colour Scheme: **`{}`".format(
                color_name
            ),
            force_document=True,
