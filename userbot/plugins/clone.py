...
"""
Credits of Plugin @ViperAdnan 
@mrconfused(revert)[will add sql soon] 
@Xditya 
@Private_45 For Editing Userbot strings
Thanks to Xditya And Telebot 
"""
...
DEFAULTUSERBIO = "Pm Protection By Marcus Userbot service = Spam+Report = die"
...
async def _(event):
    ...
    await borg.send_message(
        event.chat_id, "**COPYING THAT USER'S DP AND BIO'S**", reply_to=reply_message
    )
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"#CLONED\nSuccesfulley COPIED [{first_name}](tg://user?id={user_id })",
        )
...
async def _(event):
    ...
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID, f"#REVERT\nSuccesfully reverted back to your profile Master"
        )
